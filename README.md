...
### The one duplicated folder

`framebudget-demo/` is a copy of
[framebudget](https://github.com/eimaieros/framebudget)'s demo and source. It
exists because Cloudflare clones only this repository when it builds, so a
relative path to a sibling folder resolves to nothing in production.

Duplicated source is exactly what the `dist/` decision above avoids, and the
difference is that this copy is checked: `verificar.sh` diffs it against the
original and refuses to pass if they differ. A copy something compares is a
copy. A copy nothing compares is a second version waiting to drift.

Regenerate it with `./tools/sincronizar-framebudget.sh` after any change to the
library.
